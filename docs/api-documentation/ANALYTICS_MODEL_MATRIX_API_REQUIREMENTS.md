...
-  **World Bank API** (no key required - open access)
...
-  **County Health Rankings** (no API - CSV download)
...
-  **EPA EJScreen** (no API - CSV/shapefile download)
...
-  IRS Form 990 (covered in Domain 7)
...
-  **IRS Migration Data** (no API - CSV download)
...
-  **State Administrative Data** (varies by state)
...
-  BLS API (existing)
-  BEA API (existing)
-  **IPCC/IIASA Climate Projections** (no API - NetCDF files)
    ...
    def get_api_status(self) -> Dict[str, str]:
        ...
        for key in self.OPTIONAL_APIS:
            if key in self.keys and self.keys[key]:
                ...
                status[key] = '  Not configured (optional)'
